...
畢業專題是在建置一個以Dcard為資料來源的趨勢分析平台，透過爬蟲自動蒐集五大看板的公開貼文資料，包含標題、內文與留言。蒐集後的資料經由斷字斷詞與去除雜訊字詞等前處理步驟，並透過詞彙出現頻率萃取出熱門主題與討論焦點。為使結果更直觀易讀，本系統將分析後的資訊以文字雲、排行榜、趨勢圖與比例圖等方式視覺化呈現，並整合成一個可供使用者互動瀏覽的網站。藉由此平台使用者能更快速掌握各看板的內容趨勢與討論動向，補足原有Dcard僅依看板排序所造成的資訊不足，進一步提升資訊理解效率。
